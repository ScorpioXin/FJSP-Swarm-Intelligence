...
#随机生成初始种群
def Init_Pop(Pop_size, GS_P, LS_P, RS_P):
    """
    种群初始化
    :param Pop_size: 种群大小
    :param GS_P: MS全局选择编码的比例
    :param LS_P: MS局部选择编码的比例
    :param RS_P: MS随机选择编码的比例
    :return: MS：机器选择部分；OS：工序排序部分
    """
    # GS_P = 0.6  # 全局初始化的种群数量比例
    # LS_P = 0.2  # 局部初始化的种群数量比例
    # RS_P = 0.2  # 随机初始化的种群数量比例
    GS_Pop_size = round(Pop_size * GS_P)
    LS_Pop_size = round(Pop_size * LS_P)
    RS_Pop_size = round(Pop_size * RS_P)
    OS, MS = [], []
    pro_m, pro_t, J_num, machine_num = Pro_Data()
    for _ in range(GS_Pop_size + LS_Pop_size +RS_Pop_size):     #确保OS与MS的染色体组数相同，这样才能一一对应
        os_list = Encode.encode_OS()   #OS种群全部随机选择
        OS.append(os_list)
    for _ in range(GS_Pop_size):
        ms_list = Encode.encode_GMS(machine_num)  #MS全局选择部分
        MS.append(ms_list)
    for _ in range(LS_Pop_size):
        ms_list = Encode.encode_LMS()  #MS局部选择部分
        MS.append(ms_list)
    for _ in range(RS_Pop_size):
        ms_list = Encode.encode_RMS()  #MS随机选择部分
        MS.append(ms_list)
    return MS,OS
...
def Decode(MS,OS):
    ...
    for _ in range(len(MS)):
        ...
        for MS_idx, __ in enumerate(OS[_]):
            ...
            J_start = 0
            for i in range(__):
                J_start = J_start + OS[_].count(i)              # 在MS中某个工件首道工序开始的索引
            J_start_idx = J_start + J_appear.count(__) - 1      # 在MS中某个工件某道工序开始的索引

            idx = MS[_][J_start_idx]
...
if __name__ == "__main__":
    # 测试调度程序逻辑是否正确
    # Pro_Message_set = [[[6, 1, 6.0, 2.0], [6, 2, 1.0, 2.0], [7, 1, 6.0, 1.0], [7, 2, 4.0, 2.0], [8, 1, 6.0, 2.0], [8, 2, 3.0, 4.0], [6, 3, 3.0, 4.0], [5, 1, 2.0, 1.0], [8, 3, 1.0, 1.0], [6, 4, 2.0, 6.0], [9, 1, 6.0, 1.0], [2, 1, 2.0, 6.0], [5, 2, 1.0, 1.0], [8, 4, 2.0, 6.0], [9, 2, 1.0, 1.0], [5, 3, 2.0, 6.0], [4, 1, 1.0, 1.0], [8, 5, 2.0, 6.0], [4, 2, 2.0, 6.0], [5, 4, 3.0, 4.0], [9, 3, 4.0, 3.0], [3, 1, 2.0, 6.0], [4, 3, 3.0, 1.0], [7, 3, 3.0, 4.0], [1, 1, 3.0, 4.0], [9, 4, 1.0, 2.0], [10, 1, 6.0, 2.0], [2, 2, 3.0, 1.0], [4, 4, 2.0, 1.0], [10, 2, 3.0, 4.0], [1, 2, 2.0, 1.0], [10, 3, 2.0, 1.0], [2, 3, 1.0, 2.0], [3, 2, 6.0, 2.0], [9, 5, 3.0, 4.0], [6, 5, 1.0, 1.0], [1, 3, 6.0, 2.0], [3, 3, 1.0, 1.0], [5, 5, 4.0, 6.0], [7, 4, 5.0, 1.0], [3, 4, 3.0, 4.0], [3, 5, 1.0, 1.0], [1, 4, 1.0, 1.0], [10, 4, 6.0, 1.0], [7, 5, 3.0, 1.0], [1, 5, 3.0, 1.0], [9, 6, 2.0, 6.0], [5, 6, 3.0, 4.0], [6, 6, 4.0, 2.0], [10, 5, 2.0, 6.0], [1, 6, 4.0, 3.0], [2, 4, 2.0, 6.0], [2, 5, 1.0, 1.0], [4, 5, 6.0, 2.0], [10, 6, 4.0, 2.0]]]
    ...
    # Pop_size = 10   #种群大小
    # pro_m, pro_t, J_num, machine_num = Pro_Data()
    # MS,OS = Init_Pop(Pop_size, 0.6, 0.2, 0.2)
    # print(f'种群大小为{Pop_size}')
    # print(f"MS染色体群:{MS}")
    # print(f"OS染色体群:{OS}")