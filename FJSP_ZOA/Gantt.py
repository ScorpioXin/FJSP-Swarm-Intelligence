...
def Gantt(plt_item, fitness):
    ...
    color_set = ['red', 'Aqua', 'yellow', 'orange', 'green', 'mediumslateblue', 'purple', 'pink', 'navajowhite',
                 'Thistle', 'Magenta', 'SlateBlue', 'RoyalBlue', 'blue', 'floralwhite', 'ghostwhite', 'goldenrod',
                 'moccasin', 'navajowhite', 'navy', 'sandybrown']
    ...
    for i in range(len(plt_item)):
        ...
        plt.text(x=Start_time + ((End_time - Start_time) / 2 - 0.3), y=Machine_id - 0.05,   # 在图表中添加文本注释(x,y,s,size,fontproperties):(X轴位置,Y轴位置,字符串标签,大小,字体)
...
def Line_Chart(fitness_iter_list, iteration_num):
    ...
    plt.xlabel("迭代次数", fontproperties='SimSun', size=35)
    plt.ylabel("加工时间", fontproperties='SimSun', size=35)
    plt.tick_params(labelsize=35)           # 刻度标签的字体大小
    plt.tick_params(direction='in')         # 指定刻度线的方向
    plt.xticks(np.arange(0, iteration_num+10, 10))
    plt.yticks(np.arange(min(fitness_iter_list)-1, max(fitness_iter_list)+1, 2))
    ax = plt.gca()
    width = 2
    ax.spines['top'].set_linewidth(width)  # 设置顶部边框线的宽度为2
    ax.spines['bottom'].set_linewidth(width)
