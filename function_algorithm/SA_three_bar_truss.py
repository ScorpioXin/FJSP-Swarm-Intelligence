...
if __name__ == "__main__":
   ...
   # 历次迭代的适应度曲线
    ...
    plt.plot(iteration_num_list, iteration_fitness)
    # plt.title("首次退火的适应度曲线")
    plt.xlabel("迭代次数")
    plt.ylabel("适应度值")
    plt.xticks(np.arange(0, len(iteration_fitness), 50))
    plt.scatter(iteration_fitness.index(min(iteration_fitness)), min(iteration_fitness), color='red', marker='o', s=100)
    plt.text(iteration_fitness.index(min(iteration_fitness))+8, min(iteration_fitness)-5,
             f"f(x,y)={round(best_iteration_fitness[0], 5)}", ha='left', va='bottom')

    plt.scatter(len(iteration_fitness)-1, iteration_fitness[-1], color='blue', marker='o', s=100)
    plt.text(len(iteration_fitness)-60, iteration_fitness[-1]+5, f"f(x,y)={round(fitness, 5)}", ha='left', va='bottom')
    plt.grid()
    plt.show()
    ...
    plt.scatter(0, max(best_iteration_fitness), color='red', marker='o', s=100)
    plt.text(10, max(best_iteration_fitness)-0.05, f"f(x,y)={round(best_iteration_fitness[0], 5)}", ha='left', va='bottom')

    plt.scatter(len(best_iteration_fitness)-1, min(best_iteration_fitness), color='green', marker='o', s=100)
    plt.text(len(iteration_fitness)-100, min(best_iteration_fitness)+0.03,
